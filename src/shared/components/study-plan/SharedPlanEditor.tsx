  ...
  const { data: existingPlans } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (studentIds.length > 0) {
        const { data: students } = await supabase.from("teachngo_students").select("teachngo_id, full_name").in("teachngo_id", studentIds);
      ...
      if (allClassIds.length > 0) {
        const { data: classes } = await supabase.from("teachngo_classes").select("id, class_name").in("id", [...new Set(allClassIds)]);
