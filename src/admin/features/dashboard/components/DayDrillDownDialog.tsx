...
import { Loader2, FileText, Activity, CalendarClock, ExternalLink, BookOpenCheck, GraduationCap } from "lucide-react";
  ...
  const { data: ieltsRows, isLoading: ieltsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: tests }, { data: practices }] = await Promise.all([
        supabase
          .from("test_results")
          .select("id, created_at, total_score, user_id, test_id, tests(title)")
        ...
        supabase
          ...
          .select("id, created_at, score, user_id, exercise_id, practice_exercises(title)")
          .gte("created_at", start)
          .lte("created_at", end)
          .order("created_at", { ascending: false })
          .limit(50),
      ...
      const merged: IeltsRow[] = [
        ...((tests || []).map((t: any) => ({
          ...
          title: t.tests?.title || "(Không có tên test)",
        ...
        ...((practices || []).map((p: any) => ({
          ...
          title: p.practice_exercises?.title || "(Practice)",
  ...
  const { data: teacherRows, isLoading: teacherLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, entry_date, title, status, class_id, teachngo_classes(name)")
        .eq("entry_date", day!)
        .order("created_at", { ascending: false })
        .limit(100);
      ...
      return ((data || []) as any[]).map<TeacherRow>((r) => ({
        ...
        title: r.title,
        class_name: r.teachngo_classes?.name || null,
        class_id: r.class_id,
        status: r.status,
      }));
    },
      ...
      <DialogContent className="max-w-2xl">
        ...
        <ScrollArea className="max-h-[420px] -mx-6 px-6">
          {isLoading ? (
            ...
            <IeltsList rows={ieltsRows || []} onClickUser={(uid) => { onClose(); navigate(`/users/${uid}/performance`); }} />
          ) : (
            <TeacherList rows={teacherRows || []} onClickClass={(cid) => { onClose(); navigate(`/classes`); }} />
          )}
        </ScrollArea>
      </DialogContent>
    </Dialog>
    ...
    <div className="space-y-1.5">
      {rows.map((r) => (
        ...
        >
            ...
            <p className="text-[11px] text-muted-foreground truncate">
              {r.user_name ? (
                <button
                  ...
                  onClick={() => r.user_id && onClickUser(r.user_id)}
                >
                  {r.user_name}
                </button>
              ) : (
          ...
          {r.score != null && (
            <span className="font-bold font-mono text-sm tabular-nums shrink-0 w-10 text-right">
...
function TeacherList({ rows, onClickClass }: { rows: TeacherRow[]; onClickClass: (cid: string) => void }) {
  if (rows.length === 0) {
  ...
  return (
    <div className="space-y-1.5">
      {rows.map((r) => (
        <div key={r.id} className="flex items-center gap-3 rounded-md border bg-card px-3 py-2 text-sm hover:bg-muted/40 transition-colors">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium truncate">{r.title || "(Buổi học)"}</p>
            {r.class_name && (
              <p className="text-[11px] text-muted-foreground truncate">
                <button
                  className="hover:underline hover:text-primary"
                  onClick={() => r.class_id && onClickClass(r.class_id)}
                >
                  {r.class_name}
                </button>
              </p>
