  ...
  return (
    <div className="rounded-pop-lg border-[2.5px] border-lp-ink bg-white shadow-pop p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3 text-xs">
          <span className="flex items-center gap-1.5">
            <Flame className="h-3.5 w-3.5 text-lp-coral" />
            <span className="font-display font-extrabold text-lp-coral">{streak}</span>
            <span className="text-lp-body">streak</span>
          </span>
      ...
      <div className="flex items-center justify-between mb-3">
        <button
          onClick={() => navMonth(-1)}
          className="p-1.5 rounded-pop border-[1.5px] border-transparent text-lp-ink hover:border-lp-ink hover:bg-lp-yellow/20 transition-all"
        >
          <ChevronLeft className="h-4 w-4" />
        </button>
        <span className="text-sm font-display font-extrabold text-lp-ink">
      ...
      <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} className="touch-pan-y select-none">
        ...
        <div key={`${calYear}-${calMonth}`} className={slideDir === "right" ? "adm-slide-l" : slideDir === "left" ? "adm-slide-r" : "adm-enter"}>
          <div className="grid grid-cols-7 gap-1 mb-1">
            {WEEKDAYS.map(w => (
              <div key={w} className="text-center text-[10px] font-display font-bold uppercase text-lp-body py-1">{w}</div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-1">
            {calendarCells.map((cell, i) => {
              ...
              const el = (
                <div
                  className={cn(
                    "aspect-square rounded-pop text-xs font-display font-bold flex items-end justify-center relative overflow-hidden border-[1.5px]",
                    isToday ? "border-lp-ink shadow-pop-xs" : "border-lp-ink/10",
                    !hasAct && "text-lp-body"
                  )}
                >
                  {hasAct && (
                    <div
                      ...
                      style={{
                        height: `${barPct}%`,
                        background: `linear-gradient(to top, var(--lp-teal), var(--lp-teal-soft))`,
                  ...
                  <span className={cn(
                    ...
                    hasAct && barPct > 50 ? "text-white font-extrabold" : hasAct ? "text-lp-ink font-extrabold" : ""
                  )}>
                    {cell.day}
                  </span>
                </div>
      ...
      <div className="flex items-center gap-3 mt-3 pt-3 border-t-[2px] border-lp-ink/10 text-[10px] text-lp-body font-body">
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-pop border-[1.5px] border-lp-ink relative overflow-hidden">
            <span className="absolute inset-x-0 bottom-0 h-[30%]" style={{ background: 'linear-gradient(to top, var(--lp-teal), var(--lp-teal-soft))' }} />
          </span>
          Ít
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-pop border-[1.5px] border-lp-ink relative overflow-hidden">
            <span className="absolute inset-x-0 bottom-0 h-full" style={{ background: 'linear-gradient(to top, var(--lp-teal), var(--lp-teal-soft))' }} />
          </span>
          ≥3h
        </span>
        <span className="flex items-center gap-1.5 ml-auto">
          <span className="w-3 h-3 rounded-pop border-[1.5px] border-lp-ink shadow-pop-xs inline-block" /> Hôm nay
        </span>
      </div>
    </div>
  );
