...
import { Loader2, ShieldCheck, User, Phone, CreditCard, Users, GraduationCap, Wallet, StickyNote } from "lucide-react";
...
const GROUPS: StudentFieldGroup[] = ["basic", "contact", "personal", "guardian", "academic", "financial", "admin_notes"];
...
export default function AdminFieldAccessTab() {
  ...
  const handleToggle = async (role: string, group: string, field: "can_view" | "can_edit", current: boolean) => {
    ...
    <div className="space-y-4">
      <div>
        <h2 className="text-base font-bold flex items-center gap-2">
          ...
          Phân quyền dữ liệu học viên
        </h2>
        <p className="text-xs text-muted-foreground mt-1">
          Cấu hình quyền xem/sửa từng nhóm thông tin học viên cho mỗi vai trò
        </p>
      </div>
          ...
          <thead>
            <tr className="bg-muted/40 text-xs text-muted-foreground">
              ...
              {EDITABLE_ROLES.map(r => (
                <th key={r.key} className="text-center px-4 py-3 font-medium" colSpan={2}>{r.label}</th>
              ))}
                      ...
                      <React.Fragment key={role.key}>
                        <td className="px-2 py-3 text-center">
                          {viewSaving ? (
                            ...
                            <Switch checked={canView} onCheckedChange={() => handleToggle(role.key, group, "can_view", canView)} />
                          )}
                        </td>
                        <td className="px-2 py-3 text-center">
                          {editSaving ? (
                            ...
                            <Switch checked={canEdit} disabled={!canView} onCheckedChange={() => handleToggle(role.key, group, "can_edit", canEdit)} />
                          )}
                        </td>
                      </React.Fragment>
                    );
