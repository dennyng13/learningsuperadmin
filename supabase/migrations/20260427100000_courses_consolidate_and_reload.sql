-- ============================================================================
-- Migration: Consolidate courses/levels schema + reload PostgREST cache
-- ============================================================================
-- Date:  2026-04-27
-- Why:   Đảm bảo (idempotent) tất cả cột/bảng cần cho flow Program → Level →
--        Course → Study Plan đều tồn tại trên DB live, kể cả khi các migration
--        trước (20260427052737, 20260427073906, 20260427090000) chưa được apply
--        đầy đủ. Cuối cùng NOTIFY pgrst để PostgREST hết "Could not find the
--        'cefr' column" / "Could not find table 'courses'".
-- ============================================================================
...
-- 2) course_levels: bổ sung cột mô tả --------------------------------------
...
-- 3) program_levels: bảng + index + RLS + unique(level_id) -----------------
...
DO $$
BEGIN
  -- dedupe trước khi tạo unique(level_id)
  WITH ranked AS (
...
CREATE POLICY program_levels_select
  ON public.program_levels FOR SELECT TO authenticated USING (true);
...
BEGIN
  ...
  IF NOT FOUND THEN EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE public.program_levels'; END IF;
...
-- 9) Force PostgREST reload schema cache -----------------------------------
